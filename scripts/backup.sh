...
if [ "$#" -ne 1 ]; then
  echo "Usage: scripts/backup.sh DESTINATION_DIRECTORY" >&2
  exit 2
fi
...
mkdir -p -- "$1"
maintenance_host_dir=$(CDPATH= cd -- "$1" && pwd -P)
cd "$project_dir"

command -v docker >/dev/null 2>&1 || {
  echo "Docker is required for the Compose backup wrapper" >&2
  exit 1
}
...
restore_services() {
  ...
  if [ -n "$resume_services" ]; then
    if ! docker compose start $resume_services; then
      echo "Backup finished but the previous service state could not be restored" >&2
...
for service in $remaining; do
  if [ "$service" != "db" ]; then
    echo "Refusing backup while unexpected service '$service' is running" >&2
    exit 1
  fi
done
...
MAINTENANCE_HOST_DIR="$maintenance_host_dir" \
  ...
  node_modules/.bin/tsx src/cli/backup.ts /backups
